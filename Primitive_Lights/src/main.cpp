...
void processInput(GLFWwindow *window, glm::mat4& view_dir, glm::mat4& view_loc, float& fov, float& near_z, float& far_z);
...
int main(int argc, char *argv[])
{
    ...
    // https://wiki.ogre3d.org/tiki-index.php?page=-Point+pointlights[0]+Attenuation
    ourShader.setFloat("pointlights[0].linear_decay", 0.35f);
    ourShader.setFloat("pointlights[0].exp_decay", 0.44f);
    ourShader.setVec3("pointlights[1].specular", glm::vec3(1.0f));
    ourShader.setFloat("pointlights[1].linear_decay", 0.35f);
    ourShader.setFloat("pointlights[1].exp_decay", 0.44f);

    ourShader.setVec3("spotlights[0].ambient", glm::vec3(0.f));
    ourShader.setVec3("spotlights[0].diffuse", light3_col * 0.9f);
    ourShader.setVec3("spotlights[0].specular", glm::vec3(1.0f));
    ourShader.setFloat("spotlights[0].linear_decay", 0.35f);
    ourShader.setFloat("spotlights[0].exp_decay", 0.44f);
    ...
    float near_z = 0.1f; // near plane z
    float far_z = 100.f; // far plane z
    glm::mat4 perspective = makePerspectiveMatrix(perspective_fov, (float)SCR_WIDTH / (float)SCR_HEIGHT, near_z, far_z);
    ...
    glm::mat4 lamp3_model = glm::translate(model, light3_pos);
    lamp3_model = glm::scale(lamp3_model, glm::vec3(0.05));
    ...
    while (!glfwWindowShouldClose(window))
    {
        // input processing
        // --------------------
        processInput(window, view_dir, view_loc, perspective_fov, near_z, far_z);
        ...
        perspective = makePerspectiveMatrix(perspective_fov, (float)SCR_WIDTH / (float)SCR_HEIGHT, near_z, far_z);

        lightShader.use();
        lightShader.setMat4("view", view);
        lightShader.setMat4("perspective", perspective); 

        light1_col = glm::vec3(0.5f, 0.2f, 0.2f);
        light1_col *= sin(0.125 * glfwGetTime()) + 2;
        light1_pos = glm::vec3(1.5 + 1.5 * (sin(0.5 * glfwGetTime()) - 1.), 0., 1.2 + 1.2 * (cos(0.5 * glfwGetTime()) - 1.));
        lamp1_model = glm::translate(model, light1_pos);
        lamp1_model = glm::scale(lamp1_model, glm::vec3(0.05));

        lightShader.setMat4("model", lamp1_model);
        lightShader.setVec3("light_col", light1_col);
        cube.render();

        light2_col = glm::vec3(0.2f, 0.2f, 0.5f);
        light2_col *= sin(0.125 * glfwGetTime() + 3.14) + 2;
        light2_pos = glm::vec3(1.5 + 1.5 * (sin(0.5 * glfwGetTime() + 3.14) - 1.), 0., 1.2 + 1.2 * (cos(0.5 * glfwGetTime() + 3.14) - 1.));
        lamp2_model = glm::translate(model, light2_pos);
        lamp2_model = glm::scale(lamp2_model, glm::vec3(0.05));

        lightShader.setMat4("model", lamp2_model);
        lightShader.setVec3("light_col", light2_col);
        cube.render();

        lightShader.setMat4("model", lamp3_model);
        lightShader.setVec3("light_col", light3_col);
        cube.render();

        lightShader.setMat4("model", glm::scale(model, glm::vec3(0.05)));
        lightShader.setVec3("light_col", glm::vec3(0.f));
        cube.render();

        ourShader.use();
        ourShader.setVec3("pointlights[0].pos", light1_pos);
        ourShader.setVec3("pointlights[0].ambient", light1_col * 0.6f);
        ourShader.setVec3("pointlights[0].diffuse", light1_col * 0.9f);

        ourShader.setVec3("pointlights[1].pos", light2_pos);
        ourShader.setVec3("pointlights[1].ambient", light2_col * 0.6f);
        ourShader.setVec3("pointlights[1].diffuse", light2_col * 0.9f);
