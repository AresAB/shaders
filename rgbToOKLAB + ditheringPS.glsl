...
vec4 linear_srgb_to_oklab(vec4 c) {
    ...
    return vec4(0.2104542553 * l_ + 0.7936177850 * m_ - 0.0040720468 * s_, 1.9779984951 * l_ - 2.4285922050 * m_ + 0.4505937099 * s_, 0.0259040371 * l_ + 0.7827717662 * m_ - 0.8086757660 * s_, 1.);
}
...
void main() {
   ...
   vec4 oklab_col = linear_srgb_to_oklab(col);
   oklab_col.x = posterize(oklab_col.x, 5.);
   oklab_col.y = posterize(oklab_col.y, 2.);
   oklab_col.z = posterize(oklab_col.z, 7.);
   
   col = oklab_to_linear_srgb(oklab_col);
