...
int main(int argc, char *argv[])
{
    ...
    int tone_map_option = 0;
    int is_whitebalance = 0;
    float exposure = 1;
    float temperature  = 0;
    float tint = 0;
    float contrast = 1;
    float brightness = 0;
    ImVec4 color_filter = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);
    float saturation = 1;
    float gamma = 1;
    // render loop
    // -----------
	...
	ImGui::SliderInt("tone map option", &tone_map_option, 0, 2);
	ImGui::SliderFloat("exposure", &exposure, 0.0f, 3.0f);
	ImGui::SliderInt("whitebalance? (0 is no)", &is_whitebalance, 0, 1);
	ImGui::SliderFloat("whitebalance: temperature", &temperature, -1.0f, 1.0f);
	ImGui::SliderFloat("whitebalance: tint", &tint, -1.0f, 1.0f);
	ImGui::SliderFloat("contrast", &contrast, 0.0f, 3.0f);
	ImGui::SliderFloat("brightness", &brightness, -1.0f, 1.0f);
	ImGui::ColorEdit4("color filter", (float*)&color_filter);
	ImGui::SliderFloat("saturation", &saturation, 0.0f, 4.0f);
	ImGui::SliderFloat("gamma", &gamma, 0.0f, 4.0f);
	ImGui::End();

	ourShader.setInt("tone_map_option", tone_map_option);
	ourShader.setInt("is_whitebalance", is_whitebalance);
	ourShader.setFloat("exposure", exposure);
	ourShader.setFloat("temperature", temperature);
	ourShader.setFloat("tint", tint);
	ourShader.setFloat("contrast", contrast);
	ourShader.setFloat("brightness", brightness);
	ourShader.setVec3("color_filter", color_filter.x, color_filter.y, color_filter.z);
