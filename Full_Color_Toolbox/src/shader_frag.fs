...
uniform int tone_map_option;
uniform int is_whitebalance;
...
void main()
{
    ...
    if(is_whitebalance == 1) color = WhiteBalance(color, temperature, tint);
    ...
    // tone mapping
    if(tone_map_option == 1) color = Narkowicz_ACES(color);
    else if(tone_map_option == 2) color = Tumblin_Rushmeier(color, texture(texture1, TexCoord, 10.).rgb);
